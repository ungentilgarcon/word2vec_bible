...
python3 scripts/prepare_bible_from_docx.py --input-folder "$INPUT_DIR" --out-folder "$OUT_CSV_DIR" --artist "$ARTIST_NAME"
...
python3 scripts/train_from_folder.py --folder "$OUT_CSV_DIR" --out "$OUT_TOPOGRAM" --min-token-length 3 --light-edges --manifest "$OUT_CSV_DIR/manifest.csv"
